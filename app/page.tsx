      ...
      <aside className="sidebar">
        ...
        <div style={{marginBottom:16}}>
          ...
          {sbDays.map((d,idx)=>{const ct=C.find(x=>x.id===d.city_id);const sel=selDay===d.day_num&&view==="plan";return(
            <div key={d.id} draggable onDragStart={()=>setDragDay(idx)} onDragOver={e=>{e.preventDefault();(e.currentTarget as HTMLElement).style.borderTop="2px solid var(--accent)"}} onDragLeave={e=>{(e.currentTarget as HTMLElement).style.borderTop="none"}} onDrop={async e=>{e.preventDefault();(e.currentTarget as HTMLElement).style.borderTop="none";if(dragDay===null||dragDay===idx)return;const days=[...sbDays];const[moved]=days.splice(dragDay,1);days.splice(idx,0,moved);for(let i=0;i<days.length;i++){await supabase.from("travel_days").update({day_num:i+1}).eq("id",days[i].id)}setDragDay(null);await loadDays()}} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",borderRadius:10,background:sel?"var(--accent2)":"transparent",marginBottom:2,cursor:"grab",transition:"all .15s"}}>
        ...
        <div style={{marginBottom:16}}>
          ...
          {showHotels&&<div style={{marginTop:4}}>
            {sbDays.map(dy=>{const ct2=C.find(x2=>x2.id===dy.city_id);const booked=notes.some(n=>n.title==="booked:"+dy.id);return <div key={dy.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",borderRadius:8,marginBottom:2,background:selDay===dy.day_num?"var(--accent2)":"transparent"}}>
              ...
              {editingDay==="sh-"+dy.id?<div style={{flex:1,display:"flex",gap:4}}><input autoFocus value={dayForm.hotel} onChange={e=>setDayForm({...dayForm,hotel:e.target.value})} onKeyDown={async e=>{if(e.key==="Enter"){await supabase.from("travel_days").update({hotel:dayForm.hotel,hotel_url:"https://www.google.com/maps/search/?api=1&query="+encodeURIComponent(dayForm.hotel+", Italy")}).eq("id",dy.id);await loadDays();setEditingDay(null)}if(e.key==="Escape")setEditingDay(null)}} placeholder="Hotel..." style={{flex:1,fontSize:10,border:"1px solid var(--accent)",borderRadius:4,padding:"2px 6px",outline:"none",background:"var(--bg2)",color:"var(--text)",fontFamily:"var(--sans)"}}/></div>:<div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>{setSelDay(dy.day_num);setView("plan");setExpanded({active:"plan"})}} onDoubleClick={e=>{e.stopPropagation();setEditingDay("sh-"+dy.id);setDayForm({...dayForm,hotel:dy.hotel||""})}}>
                <div style={{fontSize:11,display:"flex",gap:4,alignItems:"baseline"}}><span style={{fontWeight:700,color:"var(--text)"}}>{dayDate(dy.day_num)}</span><span style={{color:"var(--text3)",fontSize:9}}>{ct2?.name||""}</span></div>
                {dy.hotel?<div style={{fontSize:10,color:"var(--text2)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{dy.hotel}</div>:<div style={{fontSize:10,color:"var(--text3)",fontStyle:"italic"}}>Dubbelklik om in te vullen</div>}
              </div>}
