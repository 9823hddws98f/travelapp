      ...
      <main className="main">
        ...
        {view==="plan"&&selDay&&(()=>{const sd=sbDays.find(x=>x.day_num===selDay);if(!sd)return null;const d={...sd,day:sd.day_num,cityId:sd.city_id,hotelUrl:sd.hotel_url};const c=C.find(x=>x.id===d.cityId)!;
          ...
          <div style={{animation:"fadeUp .3s ease"}}>
            ...
            {(()=>{const prev=sbDays.find(x=>x.day_num===d.day-1);const prevC=prev?C.find(x=>x.id===prev.city_id):null;const curC=c;
              const KM:Record<string,Record<string,{km:number,t:string}>>={ven:{gar:{km:180,t:"2u30"},ver:{km:120,t:"1u30"},tos:{km:260,t:"3u"},apu:{km:350,t:"4u"}},gar:{ver:{km:65,t:"45min"},apu:{km:280,t:"3u30"},tos:{km:200,t:"2u30"},ven:{km:180,t:"2u30"}},ver:{tos:{km:230,t:"2u30"},gar:{km:65,t:"45min"},ven:{km:120,t:"1u30"}},tos:{nap:{km:470,t:"4u30"},ver:{km:230,t:"2u30"},apu:{km:120,t:"1u30"}},apu:{tos:{km:120,t:"1u30"},ver:{km:200,t:"2u30"},gar:{km:280,t:"3u30"}},nap:{ama:{km:65,t:"1u15"},tos:{km:470,t:"4u30"}},ama:{nap:{km:65,t:"1u15"}}};
              const hasTravel=prevC&&prevC.id!==curC.id;const dist=hasTravel?KM[prevC.id]?.[curC.id]:null;
              const stops=d.title.includes(" - ")?d.title.split(" - ").map((s:string)=>s.trim()):hasTravel?[prevC.name,curC.name]:[curC.name];
              return <div style={{background:"var(--bg2)",borderRadius:"var(--r)",border:"1px solid var(--border)",padding:"12px 16px",marginBottom:16,boxShadow:"var(--shadow)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><span style={{fontSize:10,fontWeight:700,color:"var(--text3)",letterSpacing:1,textTransform:"uppercase"}}>{hasTravel?"Reisschema":"Locatie"}</span>{dist?<span style={{fontSize:12,color:"var(--text2)"}}>{dist.km} km — {dist.t}</span>:null}</div><div style={{display:"flex",alignItems:"center"}}>{stops.map((s:string,si:number)=><div key={si} style={{display:"flex",alignItems:"center",flex:si<stops.length-1?1:"0 0 auto"}}><div style={{display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0}}><div style={{width:si===0||si===stops.length-1?12:8,height:si===0||si===stops.length-1?12:8,borderRadius:"50%",background:si===0?"var(--accent)":si===stops.length-1?"#22c55e":"var(--bg4)",border:"2px solid "+(si===0?"var(--accent)":si===stops.length-1?"#22c55e":"var(--text3)")}}/><div style={{fontSize:10,color:si===0||si===stops.length-1?"var(--text)":"var(--text3)",marginTop:4,whiteSpace:"nowrap",fontWeight:si===0||si===stops.length-1?600:400}}>{s}</div></div>{si<stops.length-1?<div style={{flex:1,height:2,background:"var(--bg4)",margin:"0 6px",marginBottom:18,borderRadius:1}}/>:null}</div>)}</div></div>})()}
